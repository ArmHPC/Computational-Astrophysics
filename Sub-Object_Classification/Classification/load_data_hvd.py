...
from torch.utils.data import DataLoader, distributed as torch_distributed
...
def load_images(path, batch_size, domain, hvd):
    if domain == 'train':
        transform = T.Compose([
            T.ToTensor(),
    ...
    data_sampler = torch_distributed.DistributedSampler(dataset, num_replicas=hvd.size(), rank=hvd.rank())

    data_loader = DataLoader(dataset=dataset, batch_size=batch_size,
                             sampler=data_sampler, pin_memory=True, drop_last=True)
